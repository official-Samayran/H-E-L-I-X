...
class IntentRouter {
  ...
  Future<IntentType> routeInput(String input, bool isLocalAvailable) async {
    ...
    if (lowerInput.startsWith("remind me to ") || lowerInput.contains("reminder:")) {
      String title = input.toLowerCase().replaceAll("remind me to", "").replaceAll("reminder:", "").trim();
      _taskService.addTask(title, TaskType.reminder);
      return IntentType.taskExtraction;
    }
    
    if (lowerInput.contains("to do:") || lowerInput.contains("todo:") || lowerInput.contains("add to to do") || lowerInput.contains("add to todo")) {
      String title = input.toLowerCase().replaceAll("to do:", "").replaceAll("todo:", "").replaceAll("add to to do", "").replaceAll("add to todo", "").trim();
      _taskService.addTask(title, TaskType.todo);
      return IntentType.taskExtraction;
    }

    if (lowerInput.startsWith("note:") || lowerInput.contains("add to notes")) {
      String title = input.toLowerCase().replaceAll("note:", "").replaceAll("add to notes", "").trim();
